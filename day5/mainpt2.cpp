...
int main() {
    ...
    while(n == 1000000000000) {
        ...
        long int humidity = converter(i, humToLocation);
        long int temperature = converter(humidity, tempToHum);
        long int light = converter(temperature, ligthToTemp);
        long int water = converter(light, waterToLight);
        long int fertilizer = converter(water, fertToWater);
        long int soil = converter(fertilizer, soilToFert);
        long int seed = converter(soil, seedToSoil);

        for(std::vector<long int>::iterator it = seeds.begin() ; it != seeds.end(); it = it+2) {
            // long int seed = *it;
            long int seedRange = *std::next(it);
            if(*it <= seed && seed <= (*it+(seedRange-1))) {
                n = i;
                break;
            }
        }
...
long int converter(long int seed, std::vector<std::vector<long int>> list) {
    ...
    for(std::vector<std::vector<long int>>::iterator n = list.begin() ; n != list.end(); ++n) {
        ...
        if(number[0] <= seed && seed <= (number[0]+(number[2]-1))) {
            convertedNumber = number[1] + (seed - number[0]);
            break;
        }
    }
